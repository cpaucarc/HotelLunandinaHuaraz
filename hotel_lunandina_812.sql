...
INSERT INTO `detalleservicio` VALUES (1,2,1,20,'2019-12-07',1,20.00),(2,3,2,19,'2019-12-07',1,15.00);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vista_ds` AS select `ds`.`idDetServ` AS `idDetServ`,`t`.`nombreServ` AS `nombreServ`,`ds`.`cantidad` AS `cantidad`,`ds`.`subtotal` AS `precioServ`,`h`.`numHab` AS `numHab`,`p`.`DNI` AS `DNI`,concat(`p`.`apPat`,_utf8' ',`p`.`apMat`,_utf8' ',`p`.`nomb`) AS `concat(apPat,' ',apMat,' ',nomb)`,`d`.`fechaEntrada` AS `fechaEntrada`,`u`.`login` AS `login` from ((((((`detalleservicio` `ds` join `detallealojamiento` `d`) join `personas` `p`) join `clientepersona` `c`) join `habitaciones` `h`) join `tiposervicio` `t`) join `usuarios` `u` on(((`p`.`DNI` = `c`.`DNI`) and (`c`.`idclientePersona` = `d`.`idclientePersona`) and (`d`.`idhabitacion` = `h`.`idhabitacion`) and (`ds`.`idtipoServ` = `t`.`idtipoServ`) and (`ds`.`idDetAloj` = `d`.`idDetAloj`) and (`d`.`idusuario` = `u`.`idusuario`)))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-12-08 12:36:34
